...
final class UnassignFromUser extends AbstractAssignToUser
{
    ...
    {
        return UnassignFromUserAction::TYPE;
    }

    /**
     * @param \Ibexa\Segmentation\Value\Step\Action\Segment\UnassignFromUser $action
    ...
    {
        Assert::isInstanceOf($action, UnassignFromUserAction::class);
        ...
        if ($this->segmentationService->isUserAssignedToSegment($user, $valueObject)) {
            $this->segmentationService->unassignUserFromSegment($user, $valueObject);
        }
    }
}
