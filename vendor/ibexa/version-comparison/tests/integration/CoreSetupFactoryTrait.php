...
trait CoreSetupFactoryTrait
{
    ...
    {
        // @todo refactor when refactoring core SetupFactory to avoid hardcoding package path
        $coreRootId = realpath(__DIR__ . '/../../vendor/ibexa/core');
        if (false === $coreRootId) {
            throw new RuntimeException('Unable to find the ibexa/core package directory');
        }
        $settingsPath = "{$coreRootId}/src/lib/Resources/settings";
        ...
        $integrationSettingsLoader = $this->provideIntegrationSettingsLoader($coreRootId, $containerBuilder);
        ...
        $containerBuilder->setParameter('ibexa.kernel.root_dir', realpath($coreRootId));
    ...
    private function provideIntegrationSettingsLoader(
        ...
        $settingsPath = "{$coreRootId}/tests/integration/Core/Resources/settings";

        return new YamlFileLoader($containerBuilder, new FileLocator($settingsPath));
    }
}

class_alias(CoreSetupFactoryTrait::class, 'EzSystems\EzPlatformVersionComparison\Integration\Tests\CoreSetupFactoryTrait');
