...
CREATE UNIQUE INDEX IF NOT EXISTS ibexa_product_type_setting_field_definition_uidx
    ON ibexa_product_type_settings (field_definition_id, status);

CREATE INDEX IF NOT EXISTS ibexa_product_type_setting_field_definition_idx
...
CREATE UNIQUE INDEX ibexa_product_code_uidx ON ibexa_product (code);

CREATE INDEX ibexa_product_base_product_idx ON ibexa_product (base_product_id);

ALTER TABLE ibexa_product
    ADD CONSTRAINT ibexa_product_base_product_fk
        FOREIGN KEY (base_product_id) REFERENCES ibexa_product (id)
            ON UPDATE CASCADE
            ON DELETE RESTRICT
...
ALTER TABLE ibexa_product_specification
    ADD CONSTRAINT ibexa_product_fkey
...
SELECT ibexa_product_specification.code, ezcontentobject.status::bool AS status
...
UPDATE ibexa_product_specification SET product_id = ibexa_product.id
    FROM ibexa_product_specification ips
        JOIN ezcontentobject_attribute
...
WHERE ips.base_product_id IS NULL
  ...
  AND ips.id = ibexa_product_specification.id;
...
UPDATE ibexa_product_specification
...
ALTER TABLE ibexa_product_specification ALTER product_id SET NOT NULL;
...
(
    ...
    client_plain_pkce BOOLEAN NOT NULL DEFAULT false,
    PRIMARY KEY (id)
);

CREATE UNIQUE INDEX ibexa_oauth2_client_identifier_idx
    ON ibexa_oauth2_client (client_identifier);
...
CREATE INDEX ibexa_oauth2_client_redirect_uri_client_id_idx
    ON ibexa_oauth2_client_redirect_uri (client_id);

CREATE INDEX ibexa_oauth2_client_redirect_uri_client_redirect_uri_idx
    ON ibexa_oauth2_client_redirect_uri (client_redirect_uri);
...
CREATE INDEX ibexa_oauth2_client_grant_client_id_idx
    ON ibexa_oauth2_client_grant (client_id);

CREATE INDEX ibexa_oauth2_client_grant_client_grant_idx
    ON ibexa_oauth2_client_grant (client_grant);
...
CREATE INDEX ibexa_oauth2_client_token_client_id_idx
    ON ibexa_oauth2_client_token (client_id);

CREATE INDEX ibexa_oauth2_client_token_token_id_idx
    ON ibexa_oauth2_client_token (token_id);
...
CREATE INDEX ibexa_oauth2_client_scope_client_id_idx
    ON ibexa_oauth2_client_scope (client_id);

CREATE INDEX ibexa_oauth2_client_scope_client_scope_idx
    ON ibexa_oauth2_client_scope (client_scope);
...
CREATE INDEX ibexa_oauth2_token_scope_token_id_idx
    ON ibexa_oauth2_token_scope (token_id);

CREATE INDEX ibexa_oauth2_token_scope_scope_idx
    ON ibexa_oauth2_token_scope (scope);
...
CREATE INDEX ibexa_oauth2_refresh_access_token_access_token_id_idx
    ON ibexa_oauth2_refresh_access_token (access_token_id);

CREATE INDEX ibexa_oauth2_refresh_access_token_refresh_token_id_idx
    ON ibexa_oauth2_refresh_access_token (refresh_token_id);
...
CREATE INDEX ibexa_oauth2_consent_scope_consent_id_idx
    ON ibexa_oauth2_consent_scope (consent_id);

CREATE INDEX ibexa_oauth2_consent_scope_consent_scope_idx
...
CREATE INDEX ibexa_activity_log_object_class_idx
    ON ibexa_activity_log (object_class_id);

CREATE INDEX ibexa_activity_log_action_idx
    ON ibexa_activity_log (action_id);

CREATE INDEX ibexa_activity_log_object_idx
    ON ibexa_activity_log (object_id);

CREATE INDEX ibexa_activity_log_object_name_idx
    ON ibexa_activity_log (object_name);

CREATE INDEX ibexa_activity_log_group_idx
    ON ibexa_activity_log (group_id);
...
CREATE UNIQUE INDEX ibexa_activity_log_group_source_name_uidx
    ON ibexa_activity_log_group_source (name);
...
CREATE UNIQUE INDEX ibexa_activity_log_group_source_ip_uidx
    ON ibexa_activity_log_ip (ip);
...
CREATE INDEX ibexa_activity_log_group_source_idx
    ON ibexa_activity_log_group (source_id);

CREATE INDEX ibexa_activity_log_ip_idx
    ON ibexa_activity_log_group (ip_id);

CREATE INDEX ibexa_activity_log_logged_at_idx
    ON ibexa_activity_log_group (logged_at);

CREATE INDEX ibexa_activity_log_user_id_idx
    ON ibexa_activity_log_group (user_id);

COMMENT ON COLUMN ibexa_activity_log_group.logged_at IS '(DC2Type:datetime_immutable)';
...
CREATE UNIQUE INDEX ibexa_activity_log_action_uidx
    ON ibexa_activity_log_action (action);
...
CREATE UNIQUE INDEX ibexa_activity_log_object_class_uidx
    ON ibexa_activity_log_object_class (object_class);
