...
final class ProductAvailabilityVisitor extends SortClauseHandler
{
    ...
    /**
     * @phpstan-param ProductSortClauseAdapter<ProductAvailability> $sortClause
     *
     * @return string[]
     */
    public function applySelect(QueryBuilder $query, SortClause $sortClause, int $number): array
    {
        ...
        $select = sprintf('%s.availability AS %s', $table, $alias);
        if ($query->getConnection()->getDatabasePlatform() instanceof PostgreSqlDbPlatform) {
            $select = sprintf(
                'COALESCE(%s.availability, false) AS %s',
                $table,
                $alias
            );
        }
