...
final class AttributeGroupCreateStruct implements ContextProviderInterface
{
    public function accept(PolicyInterface $policy): bool
    {
        return $policy->getObject() instanceof AttributeGroupCreateStructValue;
    }

    public function getPermissionContext(PolicyInterface $policy): ContextInterface
    {
        /** @var \Ibexa\Contracts\ProductCatalog\Local\Values\AttributeGroup\AttributeGroupCreateStruct $attributeGroupCreateStruct */
        $attributeGroupCreateStruct = $policy->getObject();

        return new Context($attributeGroupCreateStruct);
    }
}
