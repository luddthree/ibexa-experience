...
use Ibexa\Contracts\ProductCatalog\Values\Catalog\CatalogCreateStruct as CatalogCreateStructValue;
use Ibexa\ProductCatalog\Local\Permission\Context;

final class CatalogCreateStruct implements ContextProviderInterface
{
    public function accept(PolicyInterface $policy): bool
    {
        return $policy->getObject() instanceof CatalogCreateStructValue;
    }

    public function getPermissionContext(PolicyInterface $policy): ContextInterface
    {
        /** @var \Ibexa\Contracts\ProductCatalog\Values\Catalog\CatalogCreateStruct $catalogCreateStruct */
        $catalogCreateStruct = $policy->getObject();

        return new Context($catalogCreateStruct);
    }
}
