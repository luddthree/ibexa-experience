...
final class AttributeDefinitionUpdateStruct implements ContextProviderInterface
{
    public function accept(PolicyInterface $policy): bool
    {
        return $policy->getObject() instanceof AttributeDefinitionUpdateStructValue;
    }

    public function getPermissionContext(PolicyInterface $policy): ContextInterface
    {
        /** @var \Ibexa\Contracts\ProductCatalog\Local\Values\AttributeDefinition\AttributeDefinitionUpdateStruct $attributeDefinitionUpdateStruct */
        $attributeDefinitionUpdateStruct = $policy->getObject();

        return new Context($attributeDefinitionUpdateStruct);
    }
}
