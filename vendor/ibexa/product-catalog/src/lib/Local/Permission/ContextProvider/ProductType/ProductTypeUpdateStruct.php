...
final class ProductTypeUpdateStruct implements ContextProviderInterface
{
    public function accept(PolicyInterface $policy): bool
    {
        return $policy->getObject() instanceof LocalProductTypeUpdateStruct;
    }

    public function getPermissionContext(PolicyInterface $policy): ContextInterface
    {
        /** @var \Ibexa\Contracts\ProductCatalog\Local\Values\ProductType\ProductTypeUpdateStruct $productTypeUpdateStruct */
        $productTypeUpdateStruct = $policy->getObject();

        return new Context($productTypeUpdateStruct->getContentTypeUpdateStruct());
    }
}
