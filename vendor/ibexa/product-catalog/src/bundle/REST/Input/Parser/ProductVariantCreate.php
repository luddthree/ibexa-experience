...
final class ProductVariantCreate extends BaseParser
{
     ...
     */
    public function parse(array $data, ParsingDispatcher $parsingDispatcher): ProductVariantCreateStruct
    {
        ...
        if (!empty($missingKeys)) {
            throw new Parser(
                sprintf(
                    'Missing properties (%s) for Product Variant.',
                    implode(', ', $missingKeys)
                )
            );
        }
        ...
        return new ProductVariantCreateStruct(
            $attributes,
            $data[self::CODE_KEY]
        );
    }
